...
-- import           Data.Digest.Murmur64         (Hashable64, asWord64, hash64, hash64Add)
import qualified Data.Map.Strict              as SM
import           Data.Text                    (Text)
import qualified Data.Text                    as T

-- import           Hayoo.FunctionInfo           (FunctionInfo(..))
import           PackageInfo            (PackageInfo(..), defPackageRank)
import           Hayoo.Hunt.IndexSchema

-- import           Holumbus.Crawler.IndexerCore (RawDoc)
...
piToDescr (PackageInfo nam ver dep aut mai cat hom syn des upl ran)
    = insDescription d'dependencies (map T.pack dep) -- add the list of dependencies
      $
